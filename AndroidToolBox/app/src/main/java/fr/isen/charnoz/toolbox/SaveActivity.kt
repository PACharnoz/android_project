...
class SaveActivity : AppCompatActivity() {
    ...
    fun save() {
        if (firstnameField.text.toString().isNotEmpty() &&
            ...
            val json = JSONObject()
            json.put(SaveActivity.firstName, firstnameField.text.toString())
            json.put(SaveActivity.lastName, lastNameField.text.toString())
            json.put(SaveActivity.bithDate, dateEditText.text.toString())
            Toast.makeText(this, json.toString(), Toast.LENGTH_LONG).show()

            val file = File(cacheDir.absolutePath+"/"+SaveActivity.fileName)
            file.writeText(json.toString())

        } else {
            Toast.makeText(this,"Tous les champs ne sont pas complet", Toast.LENGTH_LONG).show()
        }
    }
    ...
    fun read() {
        ...
        val dateString = json.getString(SaveActivity.bithDate)
